...
void on_app_activate (GtkApplication *app, gpointer user_data) {
	...
	label = gtk_label_new("Lista de temas de sonidos:");
	...
	// https://docs.gtk.org/gio/method.File.enumerate_children.html
	...
	if (!enumerator) {
		g_print("g_file_enumerate_children error: %s\n", error->message);
		g_error_free(error);
	} else {
		gtk_string_list_append (builder, "Selecciona uno");
		while(TRUE) {
			GFileInfo *info;
	...
	dat->button = gtk_button_new_with_mnemonic("_Aplicar");
