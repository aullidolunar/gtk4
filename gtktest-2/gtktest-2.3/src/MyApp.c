...
GtkWindow* my_app_get_toplevel (GApplication *app)
{
	return MY_APP (app)->win;
}
...
// UI:
	...
	g_menu_append (menu_sec2, "_Mostrar 1", "app.muestra1");
	g_menu_append (menu_sec2, "_Mostrar 2", "app.muestra2");
	g_menu_append_section (menu_model, NULL, G_MENU_MODEL (menu_sec2));
	g_menu_append (menu_sec3, "_Salir", "app.salida");
...
// Associate the toplevel to our GApplication:
	MY_APP (app)->win = GTK_WINDOW (win);

// test the association:
	gtk_window_present (MY_APP (app)->win);
